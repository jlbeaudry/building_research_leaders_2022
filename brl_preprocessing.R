# BUILDING RESEARCH LEADERS FOR EARLY-CAREER RESEARCHERS (2022)
...
df <- here::here("data", "brl_eval_raw_data.csv") %>%
  read_qualtrics(legacy = FALSE) %>%
  filter(!is.na(sc0)) %>%  # delete those who didn't answer any questions 
  select(-c("start_date":"user_language", "sc0")) %>% 
  mutate(id = 1:n()) %>% 
  relocate(id)

# [breadcrumb: check that the sc0 variable is NA for only those who skipped all items]

# load metadata 

meta <- read_csv(here::here("data", "brl_eval_metadata.csv"), lazy = FALSE) %>%
